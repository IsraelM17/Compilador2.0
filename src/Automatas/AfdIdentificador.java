...
public class AfdIdentificador {
    ...
    private boolean q0(int contador) {
        ...
        if(contador < caracter.length)
            if(caracter[contador] == '_' || (caracter[contador]>='a' && caracter[contador]<='z') || (caracter[contador]>='A' && caracter[contador]<='Z'))
                q1(contador + 1);
    ...
    private boolean q3(int contador)
    {
        aceptado = false;
        if(contador < caracter.length)
            if (caracter[contador] == '_' ||caracter[contador]>='0' && caracter[contador]<='9')
                q3(contador+1);
            else
                q3(contador+1);
        return aceptado;
